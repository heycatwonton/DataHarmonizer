...
class Toolbar {
  ...
  async validate() {
    ...
    const invalidCells = this.dh.invalid_cells

    console.log('Invalid Cells:', invalidCells);

    const validationMessagesDiv = document.querySelector('#validation-messages');

    // Clear any previous error messages
    validationMessagesDiv.innerHTML = '';

    // Iterate over invalidCells and append error messages
    for (const row in invalidCells) {
      console.log("error: " + row)
      for (const col in invalidCells[row]) {
        const errorMessage = `Invalid cell at row ${parseInt(row) + 1}, column ${invalidCells[row][col].column}: ${invalidCells[row][col].message}`;
        console.log("error message exists")
        validationMessagesDiv.innerHTML += `<p>${errorMessage}</p>`;
      }
    }

    // If any rows have error, show/hide other elements or perform actions based on errors here
    if (Object.keys(this.dh.invalid_cells).length > 0) {
      $('#next-error-button').show();
      $('#no-error-button').hide();
    } else {
      // No errors, hide error-related elements or perform other actions
