    ...
    for (let row = 0; row < this.hot.countRows(); row++) {
      ...
      for (let col = 0; col < fields.length; col++) {
        ...
        // If not an empty field, check its contents against field datatype AND/OR other kind of range
          ...
          // Preprocess the datatype to check if it falls into "integer or float" case
          // const isIntegerOrFloat = ["float", "integer"].includes(field["any_of"][0].range) || ["float", "integer"].includes(field["any_of"][1].range);
          // console.log("is integer or float?" + isIntegerOrFloat)
          ...
          switch (datatype) {
            ...
            case 'xsd:double':
              ...
              valid &&= testNumericRange(parsed, field);
              if (valid) {
                msg = 'Data type must be a double.';
              } else {
                msg = `Data type must be a double and must fall within specified range of ${field.minimum_value} to ${field.maximum_value}`;
              }
              break;
            ...
            case 'xsd:decimal':
              valid &&= testNumericRange(parsed, field);
              if (valid) {
                msg = 'Data type must be a decimal.';
              } else {
                msg = `Data type must be a decimal and must fall within specified range of ${field.minimum_value} to ${field.maximum_value}`;
              }
              break;

            case 'xsd:boolean':
              msg = 'Data type must be a boolean value, true or false';
              break;
            ...
            case 'xsd:date':
              ...
              msg = 'Data type must be a date: YYYY-MM-DD';
              break;

            case 'xsd:string':
              // Default: any string is valid.
              valid = true;
              msg = 'Data type must be a string';
              break;
          ...
          if (valid && field.pattern) {
            ...
            try {
              ...
              if (!(field.pattern in bad_pattern)) {
                ...
                msg = `Regular expression /${field.pattern}/ in ${field.title} failed`;
                console.log(
        ...
        if (!valid) {
          ...
          invalidCells[row][col] = {
            message: msg,
            row: row + 1,
            column: fields[col].title, // Add the column/slot information here
          };
          // Log the message and column information to the console
          console.log(`Invalid cell at row ${parseInt(row) + 1}, column ${fields[col].title}: ${msg}`);
        }
      ...
      isUnique.forEach((unique, uniqueIndex) => {
        ...
        if (!unique) {
          ...
          columnNumbers.forEach((columnNumber) => {
            const msg = "Value must be unique"
            invalidCells[row][columnNumber] = `Invalid cell at row ${parseInt(row) + 1}, column ${fields[columnNumber].title}: ${msg}` ;
          });
        }
      });
    };
